...
server_publishObj			=	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_publishObject.sqf";				// Создает объект в Базе Данных
server_deleteObj			=	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_deleteObj.sqf";					// Удаляет объект из Базы Данных
server_deleteObjDirect		=	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_deleteObjDirect.sqf";			// Удаляет объект из Базы Данных, NO AUTH, ВЫЗОВ ТОЛЬКО С СЕРВЕРА, NO PV ACCESS
...
// EPOCH ADDITIONS
server_swapObject		=	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_swapObject.sqf";		// Используется для Ап/Даун-грейда построек
server_publishVeh		=	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_publishVehicle.sqf";	// Используется для Генерации рандомной техники сервером
server_publishVeh2		=	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_publishVehicle2.sqf";	// Используется для Покупки техники
server_publishVeh3		=	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_publishVehicle3.sqf";	// Используется для Улучшения техники
...
server_addtoFenceUpdateArray =
{
	...
	_exitReason = switch true do {
		// Не можем использовать владельца, потому что игрок уже может быть Мертв, Не можем использовать Дистанцию, потому что игрок далеко от забора
		case (_clientKey == dayz_serverKey): {""};
		case (_index < 0): {
			format["[СЕРВЕР]: [server_functions]: Server_AddToFenceUpdateArray ОШИБКА: PUID НЕ НАЙДЕН НА СЕРВЕРЕ! PV ARRAY: %1",_this]
		};
		case ((dayz_serverClientKeys select _index) select 1 != _clientKey): {
			format["[СЕРВЕР]: [server_functions]: Server_AddToFenceUpdateArray ОШИБКА: КЛЮЧ КЛИЕНТА НЕВЕРНЫЙ ИЛИ НЕИЗВЕСТНЫЙ. PV ARRAY: %1",_this]
		};
		case !(_class isKindOf "DZ_buildables"): {
			format["[СЕРВЕР]: [server_functions]: Server_AddToFenceUpdateArray ОШИБКА: setDamage ЗАПРОШЕН БЕЗ DZ_buildable! PV ARRAY: %1",_this]
		};
		default {""};
	};
	...
	if !(_object in needUpdate_FenceObjects) then
	{
		...
		{
			diag_log format["[СЕРВЕР]: [server_functions]: УРОН: PUID(%1) запрошен setDamage %2 на Забор %3 ID:%4 UID:%5",_playerUID,_damage,_class,(_object getVariable["ObjectID","0"]),(_object getVariable["ObjectUID","0"])];
		};
	};
};
...
check_publishobject =
{
	...
	#ifdef OBJECT_DEBUG
		diag_log format["[СЕРВЕР]: [server_functions]: ОТКЛАДКА: Проверяем, Если объект: %1 разрешен, то будет опубликовано от: %2",_object,_playername];
	#endif
	...
	#ifdef OBJECT_DEBUG
		diag_log format["[СЕРВЕР]: [server_functions]: ОТКЛАДКА: Объект: %1 опубликован от: %2 было разрешено: %3",_object,_playername,_saveObject];
	#endif
...
{
	private "_data";
	//diag_log ("[БАЗА ДАННЫХ]: [server_functions]: ПОПЫТКА ЗАПИСАТЬ: " + _this);
	_data = "HiveExt" callExtension _this;
	//diag_log ("[БАЗА ДАННЫХ]: [server_functions]: ЗАПИСЬ: " +str(_data));
};
...
server_hiveReadWrite =
{
	...
	_key = _this;
	//diag_log ("[БАЗА ДАННЫХ]: [server_functions]: ПОПЫТКА ПРОЧИТАТЬ/ЗАПИСАТЬ: " + _key);
	_data = "HiveExt" callExtension _key;
	//diag_log ("[БАЗА ДАННЫХ]: [server_functions]: ЧТЕНИЕ/ЗАПИСЬ: " +str(_data));
...
server_getStatsDiff =
{
	...
	{
		diag_log format["[СЕРВЕР]: [server_functions]: Server_getStatsDiff ОШИБКА: playerUID %1 не найден на сервере!",_playerUID];
		[0,0,0,0,0]
	};
	...
	#ifdef PLAYER_DEBUG
		diag_log format["[СЕРВЕР]: [server_functions]: Server_getStatsDiff - Объект: %1 Diffs: %2 Новое: %3",_player,_result,_statsArray];
	#endif
...
dayz_recordLogin =
{
	...
	_status = switch (1==1) do {
		...
		case ((_this select 2) == 1): { "ЛОГИН АВТОРИЗОВАН, Локация: " +(_this select 4) };
		case ((_this select 2) == 2): { "ВХОД ВЫПОЛНЕН" };
		case ((_this select 2) == 3): { "ВЫХОД С СЕРВЕРА, Локация: " +(_this select 4) };
	};
	...
	diag_log format["[СЕРВЕР]: [server_functions]: ИНФОРМАЦИЯ - Игрок: %1(UID:%3/CID:%4) Статус: %2",_name,_status,(_this select 0),(_this select 1)];
};
