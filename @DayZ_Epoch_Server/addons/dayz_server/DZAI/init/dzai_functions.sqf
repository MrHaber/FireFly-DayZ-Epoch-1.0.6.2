diag_log "[СЕРВЕР]: [DZAI]: [dzai_functions]: Компиляция функций DZAI...";
...
DZAI_createGroup =
{
	...
	else
	{
		if ((resistance getFriend west) > 0) then
		{
			...
			diag_log "[СЕРВЕР]: [DZAI]: [dzai_functions]: Группа достигла максимума для East side! Изменение стороны сопротивления для использования DZAI. Рекомендуется уменьшить количество ИИ.";
		};
		resistance
	});
	//diag_log format ["[СЕРВЕР]: [DZAI]: [dzai_functions]: Выдана сторона %1 для AI группы",_groupSide];
...
DZAI_append =
{
	...
	{
		diag_log "[СЕРВЕР]: [DZAI]: [dzai_functions]: ОШИБКА: DZAI_append функция не была снабжена двумя массивами!";
	};
	{
...
DZAI_findLootPile =
{
	...
	if ((count _lootPiles) > 0) then
	{
		...
		//diag_log format ["[СЕРВЕР]: [DZAI]: [dzai_functions]: [ОТКЛАДКА]: AI группа %1 обнаружила лут и снаряжает себя более приоритетным снаряжением %2.",_unitGroup,_lootPos];
	};
};
...
DZAI_spawn_vehicle =
{
	...
	{
		diag_log format ["[СЕРВЕР]: [DZAI]: [dzai_functions]: ОШИБКА: Не удалось найти удобную точку для спавна %1 AI технки.",(_this select 0)]
	};

	if (!([(_this select 1),"vehicle"] call DZAI_checkClassname)) exitWith
	{
		diag_log format ["[СЕРВЕР]: [DZAI]: [dzai_functions]: ОШИБКА: Требуемый КлассНейм техники %1 забанен в настройках или такой КлассНейм не существует.",(_this select 1)]
	};
...
DZAI_protectGroup =
{
	...
	{
		diag_log format["[СЕРВЕР]: [DZAI]: [dzai_functions]: [РАСШИРЕННАЯ ОТКЛАДКА]: Отспавнен 1 dummy AI для сохранения группы %1.",_this];
	};
...
{
	// Используйте: [_unitGroup,_vehicle] call DZAI_respawnAIVehicle;
...
diag_log "[СЕРВЕР]: [DZAI]: [dzai_functions]: Функции DZAI были прокомпилированы.";
