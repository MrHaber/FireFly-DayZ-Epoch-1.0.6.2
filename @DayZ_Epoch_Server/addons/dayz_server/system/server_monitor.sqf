...
diag_log "[БАЗА ДАННЫХ]: [server_monitor]: ЗАПУСК...";
...
if (_outcome == "PASS") then
{
	...
	diag_log ["[СЕРВЕР]: [server_monitor]: СИНХРОНИЗАЦИЯ ВРЕМЕНИ: Локальное время установлено:", _date, "Fullmoon:",dayz_ForcefullmoonNights,"Дата получена из HiveExt.dll:",_result select 1];
...
for "_i" from 1 to 5 do {
	diag_log "[БАЗА ДАННЫХ]: [server_monitor]: Пытаемся получить объекты...";
	...
	if (typeName _result == "STRING") then
	{
		...
		diag_log ("[БАЗА ДАННЫХ]: [server_monitor]: Попытка убить процесс.. HiveExt ответ:"+str(_res));
	} else {
		diag_log ("[БАЗА ДАННЫХ]: [server_monitor]: Найдено "+str(_result select 1)+" объектов" );
		_i = 99;
	};
};
...
{
	diag_log "[БАЗА ДАННЫХ]: [server_monitor]: Ошибка соединения! Server_monitor.sqf выход.";
};	

diag_log "[БАЗА ДАННЫХ]: [server_monitor]: Отправляем запрос";
...
if (_legacyStreamingMethod) then
{
	if (_status == "ObjectStreamStart") then
	{
		...
		diag_log ("[БАЗА ДАННЫХ]: [server_monitor]: Начало потоковой передачи объектов...");
...
diag_log ("[БАЗА ДАННЫХ]: [server_monitor]: Обрабатываем поток " + str(_val) + " объектов");
...
{
	diag_log "[СЕРВЕР]: [server_monitor]: Все игроки вышли! Server_monitor.sqf выход.";
};

{
	...
	if (!_wsDone) then
	{
		if ((count _posATL) >= 2) then
		{
			...
			diag_log format["[СЕРВЕР]: [server_monitor]: ОБЪЕКТ ПЕРЕМЕЩЕН: %1 класс %2 с worldspace массивом = %3 на позицию: %4",_idKey,_type,_worldspace,_pos];
		}
		else
		{
			diag_log format["[СЕРВЕР]: [server_monitor]: ОБЪЕКТ ПЕРЕМЕЩЕН: %1 класс %2 с worldspace массивом = %3 на позицию: [0,0,0]",_idKey,_type,_worldspace];
		};
	};

	//diag_log format["[СЕРВЕР]: [server_monitor]: ОБЪЕКТ: %1 - %2,%3,%4,%5,%6,%7,%8", _idKey,_type,_ownerID,_worldspace,_inventory,_hitPoints,_fuel,_damage];
	/*
		if (_type in _tempMaint) then {
			// Используем hitpoints для системы Обслуживания и других систем позже
			// Включим Свап модели на поврежденной модели
	...
	*/
		...
		else
		{
			...
			if (_isDZ_Buildable || {(_isSafeObject && !_isTrapItem)}) then
			{
				...
				_object setVariable ["OEMPos",_pos,true]; // Используется на улучшения при установке и Закрыть/Открыть Сейф
			}
			else
			{
				_object enableSimulation true;
...
diag_log format["[БАЗА ДАННЫХ]: [server_monitor]: ОБСЛУЖИВАНИЕ - Server_monitor.sqf закончил обрабатывать %1 объектов за %2 секунд (Без планировщика)",_val,diag_tickTime - _timeStart];
...
for "_i" from 0 to 10 do {
	...
	{
		_i = 11; // Выход. Если свободных маркеров нет 
	};
};
diag_log format["[СЕРВЕР]: [server_monitor]: Всего точек спавна: %1", actualSpawnMarkerCount];
...
[] spawn
{
	...
	while {1 == 1} do {
		if ((str dayz_traps != _array) || (str dayz_traps_active != _array2) || (str dayz_traps_trigger != _array3)) then
		{
			...
			_array2		=	str dayz_traps_active;
			_array3		=	str dayz_traps_trigger;
			//diag_log "[СЕРВЕР]: [server_monitor]: ОТКЛАДКА: Ловушки - старт!";
			//diag_log format["[СЕРВЕР]: [server_monitor]:dayz_traps (%2) -> %1", dayz_traps, count dayz_traps];
			//diag_log format["[СЕРВЕР]: [server_monitor]:dayz_traps_active (%2) -> %1", dayz_traps_active, count dayz_traps_active];
			//diag_log format["[СЕРВЕР]: [server_monitor]:dayz_traps_trigger (%2) -> %1", dayz_traps_trigger, count dayz_traps_trigger];
			//diag_log "[СЕРВЕР]: [server_monitor]: ОТКЛАДКА: Ловушки - конец!";
		};
...
"PVDZ_sec_atp" addPublicVariableEventHandler {
	_x = _this select 1;
	switch (1==1) do {
		case (typeName _x == "STRING") : {	// Просто пару логов для клиента
			diag_log _x;
		};
		case (count _x == 2) : { // Неверная сторона
			diag_log format["[СЕРВЕР]: [server_monitor]: Игр0к %1 возможно получает 'side' чит (смена стороны!). Сервер может быть взломан!",(_x select 1) call fa_plr2Str];
		};
		default { // Урон
			_unit		=	_x select 0;
			_source		=	_x select 1;
			...
			{
				diag_log format ["[СЕРВЕР]: [server_monitor]: Игр0к %1 попал в %2 %3 из %4 расстояние %5 с %6 уроном",
...
"PVDZ_objgather_Knockdown" addPublicVariableEventHandler {
	...
	if (_dis < 30 && (_treeModel in dayz_trees or (_treeModel in dayz_plant)) && (_uid != "")) then
	{
		...
		diag_log format["[СЕРВЕР]: [server_monitor]: Серверный setDamage на Деревья или Посевы %1 было уронено от %2(%3)",_treeModel,_name,_uid];
	};
};
...
if (_hiveLoaded) then
{
	...
	{
		// Спавним технику
		// Получаем ВСЕ Постройки и дорогу только один раз. Очень лагает, но только при старте и первое время. Это нормально
		...
		if (_vehLimit > 0) then
		{
			diag_log ("[БАЗА ДАННЫХ]: [server_monitor]: Отспавненно техники #: " + str(_vehLimit));
			
			for "_x" from 1 to _vehLimit do {call spawn_vehicles;};
		} else
		{
			diag_log "[БАЗА ДАННЫХ]: [server_monitor]: Достигнут лимит техники!";
			_vehLimit = 0;
		};
		...
		{
			diag_log ("[БАЗА ДАННЫХ]: [server_monitor]: Отспавненно Мусора #: " + str(MaxDynamicDebris));
			for "_x" from 1 to MaxDynamicDebris do {call spawn_roadblocks;};
		};

		diag_log ("[БАЗА ДАННЫХ]: [server_monitor]: Отспавненно Ящиков #: " + str(MaxAmmoBoxes));
		for "_x" from 1 to MaxAmmoBoxes do {call spawn_ammosupply;};

		diag_log ("[БАЗА ДАННЫХ]: [server_monitor]: Отспавненно Руд #: " + str(MaxMineVeins));
		for "_x" from 1 to MaxMineVeins do {call spawn_mineveins;};

		diag_log format["[БАЗА ДАННЫХ]: [server_monitor]: ОБСЛУЖИВАНИЕ - Сервер закончил Спавн: %1 Техники, %2 Мусора, %3 Ящиков и %4 Руд в %5 секунд (С Планировщиком)",_vehLimit,MaxDynamicDebris,MaxAmmoBoxes,MaxMineVeins,diag_tickTime - _startTime];
