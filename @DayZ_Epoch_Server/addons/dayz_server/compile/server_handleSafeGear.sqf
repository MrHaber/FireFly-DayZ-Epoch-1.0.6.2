...
{
	diag_log "[СЕРВЕР]: [server_handleSafeGear]: ОШИБКА: server_handleSafeGear вызван с Нуль(Null) объектом игрока";
};
...
{
	diag_log format["[СЕРВЕР]: [server_handleSafeGear]: ОШИБКА: server_handleSafeGear вызван с Нуль(Null) объектом %1 (%2). %3 неудачная попытка.",_name,_playerUID,_statusText];
...
switch (_status) do {
	...
	case 1: {
		...
		// Сохраняем в Базу Данных (также, если игрок находится в пределах 10м в server_playerSync и server_onPlayerDisconnect)
