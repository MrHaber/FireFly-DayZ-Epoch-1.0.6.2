...
{
	diag_log ("[СЕРВЕР]: [server_playerSetup]: ОШИБКА ИНИЦИАЛИЗАЦИИ: Выход, объект игрок обнулен: " + str(_playerObj));
};
...
{
	diag_log ("[СЕРВЕР]: [server_playerSetup]: ОШИБКА ИНИЦИАЛИЗАЦИИ: Выход, игрок не имеет ID: " + str(_playerObj));
};
...
{ 
	diag_log format["[СЕРВЕР]: [server_playerSetup]: ОТКЛАДКА: _playerID не совпадает с UID! _playerID:%1",_playerID]; 
...
{
	diag_log ("[СЕРВЕР]: [server_playerSetup]: ИНИЦИАЛИЗАЦИЯ: РЕЗУЛЬТАТ: Выход, объект-игрок, обнулен: " + str(_playerObj));
};

// Ждем когда База Данных будет готова
//diag_log ("[СЕРВЕР]: [server_playerSetup]: ИНИЦИАЛИЗАЦИЯ: РЕЗУЛЬТАТ: Успешно с " + str(_primary));
...
// diag_log ("[СЕРВЕР]: [server_playerSetup]: StateNew: "+str(_statearray));
...
// diag_log ("[СЕРВЕР]: [server_playerSetup]: State: "+str(_state));
...
// diag_log ("[СЕРВЕР]: [server_playerSetup]: Достижения: "+str(_Achievements));
// diag_log ("[СЕРВЕР]: [server_playerSetup]: WORLDSPACE: " + str(_worldspace));
...
//diag_log ("[СЕРВЕР]: [server_playerSetup]: ВХОД: Локация: " + str(_worldspace) + " doRnd?: " + str(_randomSpot));
...
//		diag_log ["[СЕРВЕР]: [server_playerSetup]: Информация о персонаже: blood_type,rh_factor,testdone=",
...
//		];
	...
	else
	{
		...
		diag_log ["[СЕРВЕР]: [server_playerSetup]: Данные игрока обновлены до 1.8.3: blood_type,rh_factor=",_playerObj getVariable ["blood_type", "?"],_playerObj getVariable ["rh_factor", "?"]];
	};
}
else
{
	...
	//diag_log ["[СЕРВЕР]: [server_playerSetup]: Данные игрока: Установка: blood_type,rh_factor=",_playerObj getVariable ["blood_type", "?"],_playerObj getVariable ["rh_factor", "?"]];
...
if (_randomSpot) then
{
	...
	for [{_j = 0}, {_j <= 100 && _findSpot}, {_j = _j + 1}] do {
		...
		if ((count _position >= 2)		// !Плохая возвращаемая позиция
			&& {(_position distance _mkr < spawnArea)}) then	// !Вне зоны
			{
				...
				for [{_w = 0}, {_w != 809}, {_w = ((_w + 17) % 811)}] do {
					//if (_w < 17) then { diag_log format[ "[СЕРВЕР]: [server_playerSetup]: "%1 цикл начат с _w=%2", __FILE__, _w]; };
		...
		//diag_log format["[СЕРВЕР]: [server_playerSetup]: %1: Позиция:%2 _findSpot:%3", __FILE__, _position, _findSpot];
	};
	if (_findSpot && !_IslandMap) exitWith
	{
		diag_log format["[СЕРВЕР]: [server_playerSetup]: %1: Ошибка, Не удалось найти подходящее место для спавна игрока. Зона:%2",__FILE__, _mkr];
	};
	_worldspace = [0,_position];
	
	// Новый спавн (только реснулся), очищаем animationState когда анимация не играла при последней синхронизации
	_state = ["","reset"];
};
