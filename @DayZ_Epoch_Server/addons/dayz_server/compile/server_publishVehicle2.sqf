...
{
	diag_log "[СЕРВЕР]: [server_publishVehicle2]: Server_PublishVehicle2 ОШИБКА: Неверный формат параметра (_this < 6)";
};
...
// Данный diag_log я добавил сам чтобы проверять совпадение _clientKey (Если вам не надо, можете убрать).
diag_log format ["[СЕРВЕР]: [server_publishVehicle2]: %1,%2,%3,%4,%5,%6,%7",_worldspace,_class,_donotusekey,_keySelected,_activatingPlayer,_clientKey,_playerUID];
...
if (!_isOK) exitWith
{
	diag_log ("[БАЗА ДАННЫХ]: [server_publishVehicle2]: КЛЮЧ ДЛЯ ТЕХНИКИ НЕ СУЩЕСТВУЕТ: "+ str(_keySelected));
};
...
#ifdef OBJECT_DEBUG
	diag_log ("[БАЗА ДАННЫХ]: [server_publishVehicle2]: ЗАПИСЬ: "+ str(_key)); 
#endif
...
{
	diag_log("[СЕРВЕР]: [server_publishVehicle2]: ВЫБОРКА: Ошибка получения ID для: " + str(_uid));
}
else
{
	...
	#ifdef OBJECT_DEBUG
		diag_log("[СЕРВЕР]: [server_publishVehicle2]: ВЫБОРКА: Выбрано " + str(_oid));
	#endif
	...
	{
		// Не используйте тут setPos или CAN_COLLIDE. Он будет появляться внутри других транспортных средств
		_object = _class createVehicle _location;
		if (surfaceIsWater _location && {({_x != _object} count (_location nearEntities ["Ship",8])) == 0}) then
		{
			//createVehicle "NONE" не точен в Воде
	...
	diag_log format["[СЕРВЕР]: [server_publishVehicle2]: ПУБЛИЧНО: %1(%2) Купил %3 с UID %4 @%5",(_activatingPlayer call fa_plr2str),_playerUID,_class,_uid,(_location call fa_coor2str)];
};
