...
// Не используйте _roadList, _buildingList или _serverVehicleCounter в функции private
...
{
	diag_log "[СЕРВЕР]: [spawn_vehicles]: ОТКЛАДКА: Неудается найти удобную позицию для спавна техники";
}
else
{
	...
	if (_isShip or _isAir) then
	{
		if (_isShip) then
		{
			...
			//diag_log("[СЕРВЕР]: [spawn_vehicles]: ОТКЛАДКА: Спавним лодку около побережья " + str(_position));
		}
		else
		{
			...
			//diag_log("[СЕРВЕР]: [spawn_vehicles]: ОТКЛАДКА: Спавним Возд.технику где ровно " + str(_position));
		};
	}
	else
	{
		...
		{	
			...
			//diag_log("[СЕРВЕР]: [spawn_vehicles]: ОТКЛАДКА: Спавним рядом с дорогой " + str(_position));
		}
		else
		{
			...
			//diag_log("[СЕРВЕР]: [spawn_vehicles]: ОТКЛАДКА: Спавним рядом с постройками " + str(_position));
		};
	};
	...
	if ((count _position) == 2) then
	{
		...
		// Добавляем лут в технику (Регулируется значение)
		_num = floor(random 4);
