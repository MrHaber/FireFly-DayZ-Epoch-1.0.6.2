...
#ifdef OBJECT_DEBUG
	diag_log ("[СЕРВЕР]: [server_publishVehicle]: ПУБЛИЧНО: Попытка " + str(_object));
#endif
...
#ifdef OBJECT_DEBUG
diag_log ("[БАЗА ДАННЫХ]: [server_publishVehicle]: ЗАПИСЬ: "+ str(_key)); 
#endif
...
if (_outcome != "PASS") then
{
	...
	diag_log("[СЕРВЕР]: [server_publishVehicle]: ВЫБОРКА: Ошибка получения ID для : " + str(_uid));
} else {
	_oid = _result select 1;
	_object setVariable ["ObjectID", _oid, true];

	#ifdef OBJECT_DEBUG
	diag_log("[СЕРВЕР]: [server_publishVehicle]: ВЫБОРКА: Выбрано " + str(_oid));
	#endif
	...
	diag_log ("[СЕРВЕР]: [server_publishVehicle]: ПУБЛИЧНО: Создано " + (_class) + " с ID " + str(_uid));
};
