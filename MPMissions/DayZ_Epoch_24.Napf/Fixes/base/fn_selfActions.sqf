	...
	if (Z_singleCurrency) then {
		...
		if (_isAlive && {_typeOfCursorTarget in AllPlayers} && {isPlayer _cursorTarget}) then {
			if (s_givemoney_dialog < 0) then {
				s_givemoney_dialog = player addAction [format["<t color='#0059FF'>%1</t>",format [localize "STR_CL_ZSC_TRADE_COINS",CurrencyName,name _cursorTarget]],"Scripts\ZSC\Actions\givePlayer.sqf",_cursorTarget,3,true,true];
			};
		} else {
		...
		if (Z_globalBanking) then {
			if (_isMan && {!(isPlayer _cursorTarget)} && {_typeOfCursorTarget in ZSC_bankTraders}) then {
				if (s_bank_dialog1 < 0) then {
					s_bank_dialog1 = player addAction [localize "STR_CL_ZSC_BANK_TELLER","Scripts\ZSC\Dialogs\atmDialog.sqf",_cursorTarget,3,true,true];
				};
			} else {
			...
			if (_typeOfCursorTarget in ZSC_bankObjects) then {
				if (s_bank_dialog2 < 0) then {
					s_bank_dialog2 = player addAction [format["<t color='#0059FF'>%1</t>",localize "STR_CL_ZSC_BANK_ATM"],"Scripts\ZSC\Dialogs\atmDialog.sqf",_cursorTarget,3,true,true];
				};
			} else {
