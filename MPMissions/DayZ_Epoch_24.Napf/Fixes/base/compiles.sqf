if (isServer) then
{
	...
	fnc_veh_setFixServer		=	compile preprocessFileLineNumbers "\z\addons\dayz_code\compile\veh_setFixServer.sqf";
...
if (!isDedicated) then
{
	...
	fnc_usec_selfActions		=	compile preprocessFileLineNumbers "Fixes\base\fn_selfActions.sqf";
	...
	dayz_NutritionSystem =
	{
		...
		switch (_type) do {
			case "FoodDrink": {
				...
				{
					dayz_temperatur = dayz_temperatur + _tempCount;
				};

				//calorie
				if (_calorieCount > 0) then
				{
					dayz_nutrition = dayz_nutrition + _calorieCount;
				};

				//blood
				if (_baseRegen > 0) then
				{
					r_player_bloodregen = r_player_bloodregen + _baseRegen;
				};
			};
	...
	gear_ui_hide =
	{
		...
		if (ctrlShown (_display displayCtrl 159)) then
		{
	...
	{
		call compile preprocessFileLineNumbers "\z\addons\dayz_code\compile\player_hiveTrading.sqf";
	};
...
dayz_inflame_other =
{
	...
	if (_this select 1) then
	{
		...
		{
			_flame	=	createVehicle ["flamable_DZ", [0,0,0], [], 0, "CAN_COLLIDE"];
			...
			{
				_pos = _fireplace modelToWorld (_fireplace selectionPosition "ohniste");
				_flame setPosATL _pos;
			};
		};

		if (["matches",0.12] call fn_dynamicTool) then
		{
			_flame inflame true;
		};
	}
	else
	{
		...
		if (count _flame > 0) then
		{
			(_flame select 0) inflame false;
		};
    };
};
