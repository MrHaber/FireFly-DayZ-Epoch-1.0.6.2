...
MaxDynamicDebris 			=	1000;	// Мусор рандом на карте
...
dayz_randomMaxFuelAmount	=	300;		// Значение бензина на заправках
...
dayz_DamageMultiplier		=	2.5;		// Множитель урона Зомби (1 и 0 - Выкл)
...
DZE_selfTransfuse_Values	=	[6000,25,900]; 		// [Сколько дать крови, Шанс заражения %, Время до новой заливки]
...
DZE_SafeZoneNoBuildDistance		=	300;	// Дистанция запрета
...
DZE_PlotPole 					= 	[45,100]; 	// Радиус Плота для Базы [Сколько база, через сколько можно поставить новый плот]
DZE_BuildOnRoads 				= 	false; 		// Можно ли строить на дорогах?
DZE_BuildingLimit 				= 	1000; 		// Сколько построек можно в зоне DZE_PlotPole
DZE_BuildHeightLimit			=	25;			// Допустимая высота стройки (м)
