...
while {true} do {
	...
	{	
		...
		if (R3F_Weight < 175) then			// 175 - Вес, когда игрок моментально перестанет быть усталым. Скрипт не может хранить значение времени. Так что, пусть вес будет побольше
		{
			R3F_TIRED_Accumulator = 0;
		};
		
		if (R3F_TIRED_Accumulator > R3F_TIRED_BLACKOUT_LEVEL) then
		{
			call R3F_TIRED_FNCT_Overburdened;
		};
	}
	else
	{	
