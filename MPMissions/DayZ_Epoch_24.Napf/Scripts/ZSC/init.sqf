...
BankDialogDepositAmount =
{
	...
	{
		format[localize "STR_CL_ZSC_DEPOSIT_FAIL",CurrencyName] call dayz_rollingMessages;
	};
...
GivePlayerAmount =
{
	...
	if ((_amount < 1) or {_amount > _wealth}) exitWith
	{
		format[localize "STR_CL_ZSC_GIVE_FAIL",CurrencyName] call dayz_rollingMessages;
	};

	if (!(isPlayer ZSC_GiveMoneyTarget)) exitWith
	{
		format[localize "STR_CL_ZSC_GIVE_PLAYER",CurrencyName] call dayz_rollingMessages;
	};
